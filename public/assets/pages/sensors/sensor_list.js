var DatatablesDataSourceAjaxServer = function () {
	...
	var initTable1 = function () {
		...
		table.DataTable({
			...
			serverSide: true,
			ajax: {
				"url": "/sensors/sensor_list/get_datatables_list",
				"type": "POST"
			},
			columns: [
				{ name: 'Name', title: 'Name', data: 'name', className: 'dt-body-left' },
				{ name: 'Key', title: 'Key', data: 'key', className: 'dt-body-left' },
				{ name: 'Limit', title: 'Limit', defaultContent: '', className: 'dt-body-left' },
				{ name: 'Optimal Range', title: 'Optimal Range', defaultContent: '', className: 'dt-body-left' },
			...
			columnDefs: [
				{
					...
					render: function (data, type, full, meta) {
						return (full.lower_limit + "-" + full.uper_limit);
					},
				},
				{
					"targets": [3],
					...
					"searchable": true,
					render: function (data, type, full, meta) {
						return (full.lower_range + "-" + full.uper_range);
					},
				},
				{
					targets: -1,
...
function edit_data(row_id) {
	...
	$.ajax({
		url: "/sensors/sensor_list/details/" + row_id,
		cache: false,
		processData: false,
		contentType: 'application/json',
		type: 'GET',
...
function edit_submit() {
	...
	$.ajax({
		url: "/sensors/sensor_list/update",
		data: JSON.stringify(values),
		cache: false,
		processData: false,
		contentType: 'application/json',
...
function delete_data(row_id) {
	...
	swal({
		...
		if (ev.value) {
			$.ajax({
				url: "/sensors/sensor_list/delete",
