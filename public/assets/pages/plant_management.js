var DatatablesDataSourceAjaxServer = function () {
	...
	var initTable1 = function () {
		...
		table.DataTable({
			...
			serverSide: true,
			ajax: {
				"url": "/plant_management/get_datatables_list",
				"type": "POST"
			},
			columns: [
				{ name: 'Growing Area', title: 'Growing Area', data: 'growing_area', className: 'dt-body-left' },
				{ name: 'Plant Type', title: 'Plant Type', data: 'plant_type', className: 'dt-body-left' },
				{ name: 'Count', title: 'Count', data: 'count', className: 'dt-body-left' },
				{ name: 'Status', title: 'Status', data: 'status', className: 'dt-body-left' },
				{ name: 'Progress', title: 'Progress (%)', data: 'progress', className: 'dt-body-left' },
				{ name: 'EHD', title: 'EHD', data: 'EHD', className: 'dt-body-left' },
...
$.ajax({
	...
	success: function (dataofconfirm) {
		dataofconfirm.data.forEach(element => {
			$('#growing_area').append('<option value'+element.name+'>'+element.name+'</option>')
		});
	}
});
...
function edit_data(row_id) {
	...
	$.ajax({
		url: "/plant_management/details/" + row_id,
		cache: false,
		processData: false,
		contentType: 'application/json',
		type: 'GET',
