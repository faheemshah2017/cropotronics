var DatatablesDataSourceAjaxServer = function () {
	...
	var initTable1 = function () {
		...
		table.DataTable({
			...
			serverSide: true,
			ajax: {
				"url": "/system_controls/water_pump/get_datatables_list",
				"type": "POST"
			},
			columns: [
...
function edit_data(row_id) {
	...
	$.ajax({
		url: "/system_controls/water_pump/details/" + row_id,
		cache: false,
		processData: false,
		contentType: 'application/json',
		type: 'GET',
...
function edit_submit() {
	...
	$.ajax({
		url: "/system_controls/water_pump/update",
		data: JSON.stringify(values),
		cache: false,
		processData: false,
		contentType: 'application/json',
...
function delete_data(row_id) {
	...
	swal({
		...
		if (ev.value) {
			$.ajax({
				url: "/system_controls/water_pump/delete",
...
function toggle_active(id){
	if($('#'+id).attr('name') == 'ON'){
		...
		$.ajax({
			url: "/system_controls/water_pump/change_status/" + id,
			data: JSON.stringify(values),
			cache: false,
			processData: false,
			contentType: 'application/json',
