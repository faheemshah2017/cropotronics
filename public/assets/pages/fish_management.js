var DatatablesDataSourceAjaxServer = function () {
	...
	var initTable1 = function () {
		...
		table.DataTable({
			...
			serverSide: true,
			ajax: {
				"url": "/fish_management/get_datatables_list",
				"type": "POST"
			},
			columns: [
				{ name: 'Aquarium', title: 'Aquarium', data: 'aquarium', className: 'dt-body-left' },
				{ name: 'Fish Type', title: 'Fish Type', data: 'fish_type', className: 'dt-body-left' },
				{ name: 'Biomass', title: 'Biomass', data: 'biomass', className: 'dt-body-left' },
...
$.ajax({
	...
	success: function (dataofconfirm) {
		dataofconfirm.data.forEach(element => {
			$('#aquarium').append('<option value'+element.name+'>'+element.name+'</option>')
		});
	}
});
...
function edit_data(row_id) {
	...
	$.ajax({
		url: "/fish_management/details/" + row_id,
		cache: false,
		processData: false,
		contentType: 'application/json',
		type: 'GET',
